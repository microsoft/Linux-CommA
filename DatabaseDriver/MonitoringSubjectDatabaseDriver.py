...
class MonitoringSubjectDatabaseDriver:
    ...
    def update_revisions_for_distro(self, distro_id, new_revisions):
        ...
        if revisions_to_remove:
            print(
                "[Info] For distro: %s, deleting revisions: %s"
                % (distro_id, revisions_to_remove)
            )
            ...
            row = self.cursor.execute(
                ...
                % (cst.MONITORING_SUBJECTS_TABLE_NAME, revisions_to_remove_formatted),
                (distro_id,),
            ).fetchone()
            monitoring_subject_id = row[0]

            self.cursor.execute(
                "delete from %s where monitoringSubjectID = ?;"
                % cst.MONITORING_SUBJECTS_TABLE_NAME,
        ...
        if revisions_to_add:
            print(
                "[Info] For distro: %s, adding revisions: %s"
                % (distro_id, revisions_to_add)
            )
            for revision in revisions_to_add:
                self.cursor.execute(
                    "insert into %s ([distroID],[revision]) values(?,?)"
                    % cst.MONITORING_SUBJECTS_TABLE_NAME,
                    (distro_id, revision),
                )
                self.conx.commit()
    ...
    def get_revision_list(self, distro_id):
        rows = self.cursor.execute(
            "SELECT revision FROM %s where [distroID] = ?;"
            % cst.MONITORING_SUBJECTS_TABLE_NAME,
            (distro_id,),
        ).fetchall()
