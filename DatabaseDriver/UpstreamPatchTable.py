...
class UpstreamPatchTable():
    ...
    def insert_Upstream(self, commit_id,author_name,author_id,commit_sub,commit_msg,diff_files,commit_time,diff_fNames,author_time,diff_code):
        '''
        dump data into Upstream 
        '''
        try:
            conx = self.cursor.execute("insert into [dbo].[Upstream-PatchTracker]([patchName],[state],[commitId],[commitMessage],[author],[authorEmail],[patchFiles],[commitTime],[diff_fileNames],[authorTime]) values(?,?,?,?,?,?,?,?,?,?)",commit_sub,"Upstream",commit_id, commit_msg, author_name,author_id,diff_code,commit_time,diff_fNames,author_time)
            conx.commit()
