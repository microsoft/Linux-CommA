...
def insert_patch(db,match,distro,patch, indicator):
    if indicator == "Upstream":
        ...
        if (dict1):
            db.insertInto(dict1,distro.distro_id,patch.commit_id,patch.upstream_date, patch.buglink, distro.kernel_version, patch.author_time)   # get dirstroId from db table
